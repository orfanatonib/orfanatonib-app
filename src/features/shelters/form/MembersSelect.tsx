...
export default function MembersSelect({
  ...
  const fingerprint = useMemo(
    () =>
      (options ?? [])
        .map((o) => `${o.memberProfileId}:${o.vinculado ? 1 : 0}`)
        .join("|"),
    [options]
  );
    ...
    () =>
      (options ?? []).filter(
        (opt) => {
          ...
          return safeValue.includes(opt.memberProfileId);
        }
      ),
  ...
  const visibleOptions = useMemo(() => {
    ...
    return baseVisible.filter((opt) => {
      ...
      const id = normalize(opt.memberProfileId);
      return n.includes(q) || id.includes(q);
    });
  }, [baseVisible, q]);

  const selectedSet = useMemo(() => new Set(safeValue), [safeValue]);
  const isDisabled = (opt: MemberOption) =>
    !!opt.vinculado && !selectedSet.has(opt.memberProfileId);
  ...
  const selectVisible = useCallback(() => {
    ...
    for (const o of visibleOptions) {
      if (!isDisabled(o)) next.add(o.memberProfileId);
    }
    onChange(Array.from(next));
  }, [safeValue, visibleOptions, onChange]);

  const clearVisible = useCallback(() => {
    const toRemove = new Set(visibleOptions.map((o) => o.memberProfileId));
      ...
      if (isXs) {
        ...
        if (total <= 2) {
          const names = selected
            .map((id) => {
              const found = options.find((t) => t.memberProfileId === id);
    ...
    <FormControl fullWidth>
      ...
      >
        ...
        {visibleOptions.map((t) => {
          const isSelected = selectedSet.has(t.memberProfileId);
          const disabled = isDisabled(t);
          return (
            <MenuItem key={t.memberProfileId} value={t.memberProfileId} disabled={disabled} dense>
